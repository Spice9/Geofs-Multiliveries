...
buttonDiv.addEventListener("click", function() {
	if (typeof mlui === "object") {
		if (mlui.closed) {
			opened = false;
		}
	}
	if (opened) {
		ui.notification.show("Panel Already Open");
		return;
	}
	...
	if (geofs.version == 2.9) {
		updateMultiplayer = function() {
			Object.values(multiplayer.visibleUsers).forEach(function(e) {
				...
				livObj.aircraft.forEach(function(e) {
					if (a.currentLivery == parseInt(e.livery)) {
						geofs.api.Model.prototype.changeTexture(e.mptx, 0, multiplayer.visibleUsers[a.id].model);
					}
				})
			})
		}
		...
		updateMultiplayer = function() {
			Object.values(multiplayer.visibleUsers).forEach(function(e) {
				...
				livObj.aircraft.forEach(function(e) {
					if (a.currentLivery == parseInt(e.livery)) {
						geofs.api.changeModelTexture(multiplayer.visibleUsers[a.id].model, e.mptx, 0);
					}
				})
			})
		}
